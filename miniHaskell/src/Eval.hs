...
freshVar (IndexedVar s1 _) xs = IndexedVar s1 (maximum (fmap f xs) + 1)
                where f (IndexedVar s2 cnt) = if s1 == s2 then cnt else -1
...
renameVar t r (Lam v e) = Lam (if v == t then r else v) (renameVar t r e)
renameVar t r (App e1 e2) = App (renameVar t r e1) (renameVar t r e2)
...
substitute t r (X v) = if t == v then r else X v
substitute t r (Lam v e)
  | t == v = Lam v e
  | v `notElem` freeList = Lam v (substitute t r e)
  | otherwise = substitute t r subbedLambda
  where freeList = freeVars r
        newVar = freshVar v freeList
        subbedLambda = Lam newVar (substitute v (X newVar) e)
substitute t r (App e1 e2) = App (substitute t r e1) (substitute t r e2)
